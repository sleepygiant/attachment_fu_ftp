  ...
  module AttachmentFu # :nodoc:
    module Backends
      module FtpBackend
        ...
        def self.ftp_object
          Net::FTP.new(ftp_config[:server],ftp_config[:username],ftp_config[:password])
          # @ftp_object ||= Net::FTP.new(ftp_config[:server],ftp_config[:username],ftp_config[:password])
        end
        
        module ClassMethods
          def ftp_object
            Technoweenie::AttachmentFu::Backends::FtpBackend.ftp_object
          end
        end
        ...
        protected
          ...
          def destroy_file
            begin
              ftp_object.delete full_filename
            rescue Exception => e
